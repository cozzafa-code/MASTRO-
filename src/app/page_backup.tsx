...
export default function Dashboard() {
  ...
  // Configuratore Serramenti state
  ...
  // Load attività + serramenti when selected commessa changes + Realtime subscription
  useEffect(() => {
    if (selectedCommessa) {
  ...
  async function createSerramento(commessaId: string) {
    const nextOrdine = serramenti.length + 1
    const nextPos = `F${nextOrdine}`
    const { data, error } = await supabase.from('serramenti').insert({
      azienda_id: AZIENDA_ID, commessa_id: commessaId,
      posizione: nextPos, ambiente: '', piano: 'PT', tipo: 'finestra',
      larghezza: 1200, altezza: 1400, profondita_muro: 300,
      num_ante: 1, configurazione: '1a-ar',
      materiale: 'pvc', larghezza_profilo: 70,
      colore_esterno: '#FFFFFF', colore_interno: '#FFFFFF',
      nome_colore_esterno: 'Bianco', nome_colore_interno: 'Bianco', finitura: 'liscio',
      tipo_vetro: 'doppio', composizione_vetro: '4/16/4',
      vetro_basso_emissivo: true, gas_camera: 'argon', ug_valore: 1.1,
      tipo_ferramenta: 'standard', maniglia: 'standard',
      colore_maniglia: '#C0C0C0', posizione_maniglia: 'destra', cerniere: 'standard',
      ordine: nextOrdine,
    }).select().single()
    if (!error && data) {
      await loadSerramenti(commessaId)
      setSelectedSerramento(data.id)
      setConfigTab('dimensioni')
    }
  }

  async function updateSerramento(id: string, updates: Partial<Serramento>) {
    // Auto-calc superficie
    const curr = serramenti.find(s => s.id === id)
    if (curr) {
      const l = updates.larghezza ?? curr.larghezza
      const h = updates.altezza ?? curr.altezza
      ;(updates as any).superficie_mq = parseFloat(((l * h) / 1000000).toFixed(3))
    }
  ...
  function renderSerramentoSVG(s: Partial<Serramento>, svgW = 360, svgH = 420) {
    ...
    const config = s.configurazione || '1a-ar'
    const panels = config.split('+')
    const numP = panels.length
    const divs = s.divisione_ante ? s.divisione_ante.split(',').map(Number) : panels.map(() => 100 / numP)
    const hasTrav = s.ha_traverso || false
    const travH = hasTrav ? (s.altezza_sopraluce || 400) * scale : 0

    // Glass color based on vetro type
    const glassColor = s.tipo_vetro === 'triplo' ? 'rgba(180,210,235,0.45)' : 'rgba(190,220,245,0.35)'
    const glassReflect = 'rgba(255,255,255,0.15)'

    let svgContent = ''

    // Outer frame shadow
    svgContent += `<rect x="${ox - 2}" y="${oy - 2}" width="${w + 4}" height="${h + 4}" rx="2" fill="none" stroke="rgba(0,0,0,0.3)" stroke-width="2"/>`
    ...
    svgContent += `<rect x="${ox + tf}" y="${oy + tf}" width="${w - tf * 2}" height="${h - tf * 2}" fill="#1a1e2e" />`

    // Traverso (horizontal divider)
    if (hasTrav) {
      svgContent += `<rect x="${ox + tf}" y="${oy + h - tf - travH}" width="${w - tf * 2}" height="${tf * 0.6}" fill="${colFrame}" stroke="${colFrameDark}" stroke-width="0.5"/>`
    }

    // Draw panels
    let panelX = ox + tf
    const panelY = oy + tf
    const totalPanelW = w - tf * 2
    const mainH = hasTrav ? h - tf * 2 - travH - tf * 0.3 : h - tf * 2

    panels.forEach((panel, i) => {
      const pct = divs[i] / 100
      const pw = totalPanelW * pct - (numP > 1 && i < numP - 1 ? tf * 0.3 : 0)
      const parts = panel.trim().split('-')
      const panelType = parts[0] // 1a, 1f, etc.
      const apertura = parts[1] || 'ar' // ar, b, v, s, f
      const isFisso = panelType.includes('f')
      ...
      if (!isFisso) {
        ...
        // Glass reflection
        const gx = panelX + sf + 4
        const gy = panelY + sf + 4
        const gw = (pw - sf * 2 - 10) * 0.4
        const gh = (mainH - sf * 2 - 10) * 0.6
        svgContent += `<rect x="${gx}" y="${gy}" width="${gw}" height="${gh}" rx="1" fill="${glassReflect}" opacity="0.5"/>`
      } else {
        // Fixed panel - glass directly in frame
      ...
      // Opening symbols (DIN standard)
      if (!isFisso) {
        const cx = panelX + pw / 2
        const cy = panelY + mainH / 2
        const symW = pw * 0.35
        const symH = mainH * 0.35

        if (apertura === 'ar' || apertura === 'b') {
          // Anta-ribalta or battente: triangle from hinge to handle
          ...
          svgContent += `<polygon points="${hingeX},${panelY + sf + 2} ${hingeX},${panelY + mainH - sf - 2} ${handleX},${panelY + mainH / 2}" fill="none" stroke="rgba(100,150,200,0.5)" stroke-width="1" stroke-dasharray="4,2"/>`
          
          if (apertura === 'ar') {
            // Additional vasistas triangle for anta-ribalta
            svgContent += `<polygon points="${panelX + sf + 2},${panelY + mainH - sf} ${panelX + pw - sf - 2},${panelY + mainH - sf} ${cx},${panelY + sf + 2}" fill="none" stroke="rgba(100,200,150,0.4)" stroke-width="0.8" stroke-dasharray="3,2"/>`
          }
        } else if (apertura === 'v') {
          // Vasistas: triangle from bottom
          svgContent += `<polygon points="${panelX + sf + 2},${panelY + mainH - sf} ${panelX + pw - sf - 2},${panelY + mainH - sf} ${cx},${panelY + sf + 4}" fill="none" stroke="rgba(100,150,200,0.5)" stroke-width="1" stroke-dasharray="4,2"/>`
        } else if (apertura === 's') {
          // Scorrevole: arrows
          svgContent += `<line x1="${cx - symW}" y1="${cy}" x2="${cx + symW}" y2="${cy}" stroke="rgba(100,150,200,0.5)" stroke-width="1.5"/>`
          svgContent += `<polygon points="${cx + symW},${cy} ${cx + symW - 8},${cy - 5} ${cx + symW - 8},${cy + 5}" fill="rgba(100,150,200,0.5)"/>`
        }

        // Handle
        if (apertura !== 's' && apertura !== 'v') {
          const hx = s.posizione_maniglia === 'sinistra' ? panelX + sf + 3 : panelX + pw - sf - 3
          const hy = panelY + mainH / 2
      ...
      if (i < numP - 1) {
        panelX += pw
        svgContent += `<rect x="${panelX}" y="${panelY}" width="${tf * 0.3}" height="${mainH}" fill="${colFrame}" stroke="${colFrameDark}" stroke-width="0.3"/>`
        panelX += tf * 0.3
      }
    })

    // Sopraluce glass (if traverso)
    if (hasTrav) {
      const slY = oy + h - tf - travH + tf * 0.3
      const slH = travH - tf * 0.3
      svgContent += `<rect x="${ox + tf + 1}" y="${slY + 1}" width="${totalPanelW - 2}" height="${slH - 2}" fill="${glassColor}" stroke="rgba(100,130,160,0.3)" stroke-width="0.5"/>`
      if (s.tipo_sopraluce === 'vasistas') {
        const scx = ox + w / 2
        svgContent += `<polygon points="${ox + tf + 4},${slY + slH - 2} ${ox + w - tf - 4},${slY + slH - 2} ${scx},${slY + 4}" fill="none" stroke="rgba(100,150,200,0.4)" stroke-width="0.8" stroke-dasharray="3,2"/>`
      }
    }

    // Dimensions
    const dimStyle = 'fill: #5b9cf6; font-size: 10px; font-family: monospace; text-anchor: middle;'
    // Width dimension
    svgContent += `<line x1="${ox}" y1="${oy + h + 15}" x2="${ox + w}" y2="${oy + h + 15}" stroke="#5b9cf6" stroke-width="0.8"/>`
    svgContent += `<line x1="${ox}" y1="${oy + h + 10}" x2="${ox}" y2="${oy + h + 20}" stroke="#5b9cf6" stroke-width="0.8"/>`
    svgContent += `<line x1="${ox + w}" y1="${oy + h + 10}" x2="${ox + w}" y2="${oy + h + 20}" stroke="#5b9cf6" stroke-width="0.8"/>`
    svgContent += `<text x="${ox + w / 2}" y="${oy + h + 28}" style="${dimStyle}">${L} mm</text>`
    // Height dimension
    svgContent += `<line x1="${ox + w + 15}" y1="${oy}" x2="${ox + w + 15}" y2="${oy + h}" stroke="#5b9cf6" stroke-width="0.8"/>`
    svgContent += `<line x1="${ox + w + 10}" y1="${oy}" x2="${ox + w + 20}" y2="${oy}" stroke="#5b9cf6" stroke-width="0.8"/>`
    svgContent += `<line x1="${ox + w + 10}" y1="${oy + h}" x2="${ox + w + 20}" y2="${oy + h}" stroke="#5b9cf6" stroke-width="0.8"/>`
  ...
  const CommesseContent = () => (
    <div>
      ...
      <div className="grid gap-6" style={{ gridTemplateColumns: commessaView === 'configuratore' && selectedCommessa ? '1fr' : selectedCommessa ? '1fr 420px' : '1fr 300px' }}>
        {/* Commesse List - hidden in configuratore mode */}
        {commessaView !== 'configuratore' && <div>
          <Card>
            <div className="flex items-center justify-between mb-4" style={{ borderBottom: '1px solid #1a1e2e', paddingBottom: 12 }}>
          ...
          {selectedCommessa && commesse.find(c => c.id === selectedCommessa) ? (() => {
            ...
            return (
              <div className="space-y-3">
                ...
                {/* View toggle: Progetto / Configuratore */}
                <div className="flex gap-1 p-1 rounded-lg" style={{ background: '#0d0f17', display: 'inline-flex' }}>
                  {[{ id: 'progetto', label: '📊 Progetto', count: commessaAttivita.length }, { id: 'configuratore', label: '🪟 Configuratore', count: serramenti.length }].map(v => (
                    <button key={v.id} onClick={() => setCommessaView(v.id as any)} className="px-3 py-1.5 rounded-md text-xs font-semibold"
                      style={{ background: commessaView === v.id ? '#5b9cf620' : 'transparent', color: commessaView === v.id ? '#5b9cf6' : '#5a5f75', border: 'none', cursor: 'pointer' }}>
                      {v.label} <span style={{ fontFamily: 'monospace', marginLeft: 4, fontSize: 10 }}>{v.count}</span>
                    </button>
                  ))}
                </div>

                {/* ===== CONFIGURATORE SERRAMENTI ===== */}
                {commessaView === 'configuratore' && (
                  <div className="grid gap-4" style={{ gridTemplateColumns: '240px 1fr 320px' }}>
                    {/* Left: Lista serramenti */}
                    <Card>
                      <div className="flex items-center justify-between mb-3">
                        <h4 style={{ fontSize: 11, fontWeight: 700, color: '#5b9cf6', textTransform: 'uppercase' as const, letterSpacing: 1 }}>SERRAMENTI</h4>
                        <button onClick={() => createSerramento(sel.id)} style={{ background: '#5b9cf620', border: '1px solid #5b9cf640', borderRadius: 6, padding: '2px 8px', fontSize: 11, color: '#5b9cf6', cursor: 'pointer', fontWeight: 600 }}>+</button>
                      </div>
                      <div className="space-y-1.5" style={{ maxHeight: 500, overflowY: 'auto' }}>
                        {serramenti.map((s, i) => {
                          const tipoInfo = TIPI_SERRAMENTO.find(t => t.value === s.tipo) || TIPI_SERRAMENTO[0]
                          const isActive = selectedSerramento === s.id
                          return (
                            <div key={s.id} onClick={() => setSelectedSerramento(isActive ? null : s.id)}
                              className="p-2 rounded-lg cursor-pointer" style={{ background: isActive ? '#5b9cf610' : '#0d0f17', border: `1px solid ${isActive ? '#5b9cf640' : '#1a1e2e'}` }}>
                              <div className="flex items-center gap-2">
                                <span style={{ fontSize: 16 }}>{tipoInfo.icon}</span>
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-center gap-1">
                                    <span style={{ fontSize: 11, fontFamily: 'monospace', fontWeight: 700, color: '#f5a623' }}>{s.posizione || `#${i + 1}`}</span>
                                    <span style={{ fontSize: 10, color: '#eae8e3' }}>{s.ambiente || tipoInfo.label}</span>
                                  </div>
                                  <div style={{ fontSize: 9, color: '#5a5f75', fontFamily: 'monospace' }}>
                                    {s.larghezza}×{s.altezza} mm · {s.num_ante}A · {MATERIALI_PROFILO.find(m => m.value === s.materiale)?.label || s.materiale}
                                  </div>
                                </div>
                                <button onClick={(e) => { e.stopPropagation(); deleteSerramento(s.id) }} style={{ color: '#5a5f75', fontSize: 10, background: 'transparent', border: 'none', cursor: 'pointer' }}>🗑</button>
                              </div>
                              {/* Mini preview */}
                              <div className="mt-1.5 flex justify-center" dangerouslySetInnerHTML={{ __html: renderSerramentoSVG(s, 200, 120) }} />
                            </div>
                          )
                        })}
                        {serramenti.length === 0 && (
                          <div className="text-center py-8">
                            ...
                            <p style={{ fontSize: 11, color: '#5a5f75', marginTop: 8 }}>Nessun serramento</p>
                            <button onClick={() => createSerramento(sel.id)} className="mt-3 px-4 py-2 rounded-lg text-xs font-semibold"
                              style={{ background: '#5b9cf6', color: '#08090d' }}>+ Aggiungi Serramento</button>
                          </div>
                        )}
                      </div>
                      {serramenti.length > 0 && (
                        <div className="mt-3 pt-2" style={{ borderTop: '1px solid #1a1e2e', fontSize: 10, color: '#5a5f75' }}>
                          <div className="flex justify-between"><span>Totale serramenti:</span><span style={{ fontFamily: 'monospace', color: '#eae8e3' }}>{serramenti.length}</span></div>
                          <div className="flex justify-between"><span>Superficie totale:</span><span style={{ fontFamily: 'monospace', color: '#eae8e3' }}>{serramenti.reduce((a, s) => a + (s.superficie_mq || (s.larghezza * s.altezza / 1000000)), 0).toFixed(2)} m²</span></div>
                          {serramenti.some(s => s.prezzo_totale > 0) && <div className="flex justify-between"><span>Valore:</span><span style={{ fontFamily: 'monospace', color: '#34d399' }}>{fmt(serramenti.reduce((a, s) => a + (s.prezzo_totale || 0), 0))}</span></div>}
                        </div>
                      )}
                    </Card>
                    ...
                    {/* Center: SVG Preview */}
                    <Card>
                      {selectedSerramento && serramenti.find(s => s.id === selectedSerramento) ? (() => {
                        const currS = serramenti.find(s => s.id === selectedSerramento)!
                        return (
                          <div>
                            <div className="flex items-center justify-between mb-2">
                              <h4 style={{ fontSize: 11, fontWeight: 700, color: '#eae8e3' }}>ANTEPRIMA</h4>
                              <div style={{ fontSize: 9, color: '#5a5f75', fontFamily: 'monospace' }}>
                                Scala 1:{Math.round(currS.larghezza / 360 * 10) / 10}
                              </div>
                            </div>
                            <div className="flex justify-center items-center" style={{ background: '#0d0f17', borderRadius: 12, padding: 16, minHeight: 460 }}
                              dangerouslySetInnerHTML={{ __html: renderSerramentoSVG(currS, 400, 440) }} />
                            {/* Quick info bar */}
                            <div className="mt-3 grid grid-cols-4 gap-2">
                              {[
                                { label: 'Tipo', value: TIPI_SERRAMENTO.find(t => t.value === currS.tipo)?.label || currS.tipo },
                                { label: 'Materiale', value: MATERIALI_PROFILO.find(m => m.value === currS.materiale)?.label || currS.materiale },
                                { label: 'Vetro', value: currS.composizione_vetro || '4/16/4' },
                                { label: 'Superficie', value: `${((currS.larghezza * currS.altezza) / 1000000).toFixed(2)} m²` },
                              ].map(info => (
                                <div key={info.label} className="text-center p-2 rounded-lg" style={{ background: '#0d0f17' }}>
                                  <div style={{ fontSize: 8, color: '#5a5f75', textTransform: 'uppercase' as const }}>{info.label}</div>
                                  <div style={{ fontSize: 10, color: '#eae8e3', fontWeight: 600 }}>{info.value}</div>
                                </div>
                              ))}
                            </div>
                          </div>
                        ...
                        <div className="flex flex-col items-center justify-center py-20" style={{ color: '#5a5f75' }}>
                          <div style={{ fontSize: 60 }}>🪟</div>
                          <p style={{ fontSize: 12, marginTop: 12 }}>Seleziona un serramento</p>
                          <p style={{ fontSize: 10, marginTop: 4 }}>per vedere l&apos;anteprima</p>
                        </div>
                      )}
                    </Card>
                    ...
                    <Card>
                      {selectedSerramento && serramenti.find(s => s.id === selectedSerramento) ? (() => {
                        const currS = serramenti.find(s => s.id === selectedSerramento)!
                        const upd = (field: string, value: any) => updateSerramento(currS.id, { [field]: value } as any)
                        ...
                        return (
                          <div>
                            ...
                            <div className="flex gap-1 mb-3 flex-wrap">
                              {[
                                { id: 'dimensioni', label: '📐 Misure' },
                                { id: 'apertura', label: '🔄 Apertura' },
                                { id: 'profilo', label: '🎨 Profilo' },
                                { id: 'vetro', label: '💎 Vetro' },
                                { id: 'ferramenta', label: '🔧 Ferramenta' },
                                { id: 'accessori', label: '📦 Accessori' },
                              ].map(t => (
                                <button key={t.id} onClick={() => setConfigTab(t.id)} style={{
                                  padding: '4px 8px', borderRadius: 6, fontSize: 9, border: 'none', cursor: 'pointer',
                                  background: configTab === t.id ? '#5b9cf620' : '#0d0f17',
                                  color: configTab === t.id ? '#5b9cf6' : '#5a5f75', fontWeight: configTab === t.id ? 700 : 400,
                                }}>{t.label}</button>
                              ))}
                            </div>

                            {/* ---- DIMENSIONI ---- */}
                            {configTab === 'dimensioni' && (
                              <div className="space-y-3">
                                <div className="grid grid-cols-2 gap-2">
                                  <InputField label="Posizione" value={currS.posizione} onChange={v => upd('posizione', v)} placeholder="es. F1, PF2" />
                                  <InputField label="Ambiente" value={currS.ambiente} onChange={v => upd('ambiente', v)} placeholder="es. Soggiorno" />
                                </div>
                                <SelectField label="Piano" value={currS.piano} onChange={v => upd('piano', v)} options={[
                                  { value: 'PS', label: 'Piano Seminterrato' }, { value: 'PT', label: 'Piano Terra' },
                                  { value: 'P1', label: 'Primo Piano' }, { value: 'P2', label: 'Secondo Piano' }, { value: 'P3', label: 'Terzo Piano' },
                                ]} />
                                <div>
                                  <label style={{ fontSize: 9, color: '#5a5f75', fontWeight: 600, textTransform: 'uppercase' as const, letterSpacing: 0.5, marginBottom: 4, display: 'block' }}>TIPO SERRAMENTO</label>
                                  <div className="grid grid-cols-2 gap-1.5">
                                    {TIPI_SERRAMENTO.map(t => (
                                      <button key={t.value} onClick={() => upd('tipo', t.value)}
                                        className="p-2 rounded-lg text-left" style={{
                                          background: currS.tipo === t.value ? '#5b9cf615' : '#0d0f17',
                                          border: `1px solid ${currS.tipo === t.value ? '#5b9cf640' : '#1a1e2e'}`,
                                          cursor: 'pointer',
                                        }}>
                                        <span style={{ fontSize: 14 }}>{t.icon}</span>
                                        <span style={{ fontSize: 9, color: currS.tipo === t.value ? '#5b9cf6' : '#8b90a8', marginLeft: 4, fontWeight: currS.tipo === t.value ? 700 : 400 }}>{t.label}</span>
                                      </button>
                                    ))}
                                  </div>
                                </div>
                                <div className="grid grid-cols-3 gap-2">
                                  <div>
                                    <label style={{ fontSize: 9, color: '#5a5f75', fontWeight: 600, textTransform: 'uppercase' as const, display: 'block', marginBottom: 4 }}>LARGHEZZA mm</label>
                                    <input type="number" value={currS.larghezza} onChange={e => upd('larghezza', parseInt(e.target.value) || 0)} step={10}
                                      style={{ width: '100%', background: '#0d0f17', border: '1px solid #252a3d', borderRadius: 8, padding: '8px', fontSize: 14, color: '#eae8e3', fontFamily: 'monospace', fontWeight: 700, textAlign: 'center', outline: 'none' }} />
                                  </div>
                                  <div>
                                    <label style={{ fontSize: 9, color: '#5a5f75', fontWeight: 600, textTransform: 'uppercase' as const, display: 'block', marginBottom: 4 }}>ALTEZZA mm</label>
                                    <input type="number" value={currS.altezza} onChange={e => upd('altezza', parseInt(e.target.value) || 0)} step={10}
                                      style={{ width: '100%', background: '#0d0f17', border: '1px solid #252a3d', borderRadius: 8, padding: '8px', fontSize: 14, color: '#eae8e3', fontFamily: 'monospace', fontWeight: 700, textAlign: 'center', outline: 'none' }} />
                                  </div>
                                  <div>
                                    <label style={{ fontSize: 9, color: '#5a5f75', fontWeight: 600, textTransform: 'uppercase' as const, display: 'block', marginBottom: 4 }}>PROF. MURO mm</label>
                                    <input type="number" value={currS.profondita_muro} onChange={e => upd('profondita_muro', parseInt(e.target.value) || 0)} step={10}
                                      style={{ width: '100%', background: '#0d0f17', border: '1px solid #252a3d', borderRadius: 8, padding: '8px', fontSize: 14, color: '#eae8e3', fontFamily: 'monospace', fontWeight: 700, textAlign: 'center', outline: 'none' }} />
                                  </div>
                                </div>
                                {/* Quick dimension presets */}
                                <div>
                                  <label style={{ fontSize: 9, color: '#5a5f75', fontWeight: 600, marginBottom: 4, display: 'block' }}>MISURE RAPIDE</label>
                                  <div className="flex gap-1 flex-wrap">
                                    {[
                                      { l: 600, h: 800, label: '60×80' }, { l: 800, h: 1000, label: '80×100' },
                                      { l: 1000, h: 1200, label: '100×120' }, { l: 1200, h: 1400, label: '120×140' },
                                      { l: 1400, h: 1600, label: '140×160' }, { l: 1600, h: 2200, label: '160×220' },
                                      { l: 1800, h: 2200, label: '180×220' }, { l: 2400, h: 2200, label: '240×220' },
                                    ].map(p => (
                                      <button key={p.label} onClick={() => { upd('larghezza', p.l); setTimeout(() => upd('altezza', p.h), 100) }}
                                        style={{ padding: '3px 6px', borderRadius: 4, fontSize: 8, border: '1px solid #252a3d', background: '#0d0f17', color: '#8b90a8', cursor: 'pointer', fontFamily: 'monospace' }}>
                                        {p.label}
                                      </button>
                                    ))}
                                  </div>
                                ...
                                {/* Traverso */}
                                <div className="flex items-center gap-3 p-2 rounded-lg" style={{ background: '#0d0f17' }}>
                                  <button onClick={() => upd('ha_traverso', !currS.ha_traverso)}
                                    style={{ width: 32, height: 18, borderRadius: 9, border: 'none', cursor: 'pointer', position: 'relative', background: currS.ha_traverso ? '#5b9cf6' : '#252a3d' }}>
                                    <div style={{ width: 14, height: 14, borderRadius: 7, background: '#eae8e3', position: 'absolute', top: 2, left: currS.ha_traverso ? 16 : 2, transition: 'left 0.2s' }} />
                                  </button>
                                  <span style={{ fontSize: 10, color: '#8b90a8' }}>Traverso / Sopraluce</span>
                                  {currS.ha_traverso && (
                                    <input type="number" value={currS.altezza_sopraluce} onChange={e => upd('altezza_sopraluce', parseInt(e.target.value) || 0)}
                                      style={{ width: 70, background: '#181b27', border: '1px solid #252a3d', borderRadius: 4, padding: '2px 4px', fontSize: 10, color: '#eae8e3', fontFamily: 'monospace', textAlign: 'center', outline: 'none' }} />
                                  )}
                                </div>
                              </div>
                            )}
                            ...
                            {configTab === 'apertura' && (
                              <div className="space-y-3">
                                <SelectField label="Numero Ante" value={String(currS.num_ante)} onChange={v => {
                                  const n = parseInt(v)
                                  const configs: Record<number, string> = { 1: '1a-ar', 2: '1a-ar+1a-ar', 3: '1a-ar+1a-ar+1a-ar' }
                                  upd('num_ante', n)
                                  setTimeout(() => upd('configurazione', configs[n] || '1a-ar'), 100)
                                }} options={[{ value: '1', label: '1 Anta' }, { value: '2', label: '2 Ante' }, { value: '3', label: '3 Ante' }]} />
                                <div>
                                  <label style={{ fontSize: 9, color: '#5a5f75', fontWeight: 600, textTransform: 'uppercase' as const, letterSpacing: 0.5, marginBottom: 6, display: 'block' }}>CONFIGURAZIONE</label>
                                  <div className="grid grid-cols-2 gap-1.5">
                                    {[
                                      { value: '1a-ar', label: '1 Anta-Ribalta', icon: '↕🔄' },
                                      { value: '1a-b', label: '1 Battente', icon: '↔️' },
                                      { value: '1a-v', label: '1 Vasistas', icon: '⬆️' },
                                      { value: '1f', label: '1 Fisso', icon: '⬜' },
                                      { value: '1f+1a-ar', label: 'Fisso + A-R', icon: '⬜↕' },
                                      { value: '1a-ar+1f', label: 'A-R + Fisso', icon: '↕⬜' },
                                      { value: '1a-ar+1a-ar', label: '2 Ante A-R', icon: '↕↕' },
                                      { value: '1a-b+1a-b', label: '2 Battenti', icon: '↔↔' },
                                      { value: '1a-s+1a-s', label: '2 Scorrevoli', icon: '⇄' },
                                      { value: '1f+1a-ar+1f', label: 'F+A-R+F', icon: '⬜↕⬜' },
                                    ].map(c => (
                                      <button key={c.value} onClick={() => {
                                        upd('configurazione', c.value)
                                        setTimeout(() => upd('num_ante', c.value.split('+').length), 100)
                                      }}
                                        className="p-2 rounded-lg text-left" style={{
                                          background: currS.configurazione === c.value ? '#5b9cf615' : '#0d0f17',
                                          border: `1px solid ${currS.configurazione === c.value ? '#5b9cf640' : '#1a1e2e'}`,
                                          cursor: 'pointer',
                                        }}>
                                        <span style={{ fontSize: 12 }}>{c.icon}</span>
                                        <div style={{ fontSize: 8, color: currS.configurazione === c.value ? '#5b9cf6' : '#8b90a8', fontWeight: currS.configurazione === c.value ? 700 : 400, marginTop: 2 }}>{c.label}</div>
                                      </button>
                                    ))}
                                  </div>
                                </div>
                                <SelectField label="Posizione Maniglia" value={currS.posizione_maniglia} onChange={v => upd('posizione_maniglia', v)}
                                  options={[{ value: 'destra', label: 'Destra (DX)' }, { value: 'sinistra', label: 'Sinistra (SX)' }]} />
                                {currS.num_ante > 1 && (
                                  <InputField label="Divisione ante (%)" value={currS.divisione_ante || ''} onChange={v => upd('divisione_ante', v)} placeholder="es. 40,60" />
                                )}
                              </div>
                            )}
                            ...
                            {configTab === 'profilo' && (
                              <div className="space-y-3">
                                <div>
                                  <label style={{ fontSize: 9, color: '#5a5f75', fontWeight: 600, textTransform: 'uppercase' as const, letterSpacing: 0.5, marginBottom: 6, display: 'block' }}>MATERIALE</label>
                                  <div className="grid grid-cols-2 gap-1.5">
                                    {MATERIALI_PROFILO.map(m => (
                                      <button key={m.value} onClick={() => upd('materiale', m.value)}
                                        className="flex items-center gap-2 p-2 rounded-lg" style={{
                                          background: currS.materiale === m.value ? '#5b9cf615' : '#0d0f17',
                                          border: `1px solid ${currS.materiale === m.value ? '#5b9cf640' : '#1a1e2e'}`,
                                          cursor: 'pointer',
                                        }}>
                                        <div className="w-5 h-5 rounded-full" style={{ background: m.color, border: '1px solid #5a5f75' }} />
                                        <span style={{ fontSize: 10, color: currS.materiale === m.value ? '#5b9cf6' : '#8b90a8', fontWeight: currS.materiale === m.value ? 700 : 400 }}>{m.label}</span>
                                      </button>
                                    ))}
                                  </div>
                                </div>
                                <InputField label="Sistema Profilo" value={currS.sistema_profilo} onChange={v => upd('sistema_profilo', v)} placeholder="es. Schüco AWS 75, Rehau Synego" />
                                <div>
                                  <label style={{ fontSize: 9, color: '#5a5f75', fontWeight: 600, textTransform: 'uppercase' as const, letterSpacing: 0.5, marginBottom: 6, display: 'block' }}>COLORE ESTERNO</label>
                                  <div className="grid grid-cols-3 gap-1.5">
                                    {COLORI_SERRAMENTO.map(c => (
                                      <button key={c.value} onClick={() => { upd('colore_esterno', c.value); setTimeout(() => upd('nome_colore_esterno', c.label), 50) }}
                                        className="flex items-center gap-1.5 p-1.5 rounded-lg" style={{
                                          background: currS.colore_esterno === c.value ? '#5b9cf615' : '#0d0f17',
                                          border: `1px solid ${currS.colore_esterno === c.value ? '#5b9cf640' : '#1a1e2e'}`,
                                          cursor: 'pointer',
                                        }}>
                                        <div className="w-4 h-4 rounded" style={{ background: c.value, border: '1px solid #5a5f75' }} />
                                        <span style={{ fontSize: 8, color: currS.colore_esterno === c.value ? '#eae8e3' : '#8b90a8' }}>{c.label}</span>
                                      </button>
                                    ))}
                                  </div>
                                </div>
                                <div>
                                  <label style={{ fontSize: 9, color: '#5a5f75', fontWeight: 600, textTransform: 'uppercase' as const, letterSpacing: 0.5, marginBottom: 6, display: 'block' }}>COLORE INTERNO</label>
                                  <div className="grid grid-cols-3 gap-1.5">
                                    {COLORI_SERRAMENTO.slice(0, 6).map(c => (
                                      <button key={c.value} onClick={() => { upd('colore_interno', c.value); setTimeout(() => upd('nome_colore_interno', c.label), 50) }}
                                        className="flex items-center gap-1.5 p-1.5 rounded-lg" style={{
                                          background: currS.colore_interno === c.value ? '#5b9cf615' : '#0d0f17',
                                          border: `1px solid ${currS.colore_interno === c.value ? '#5b9cf640' : '#1a1e2e'}`,
                                          cursor: 'pointer',
                                        }}>
                                        <div className="w-4 h-4 rounded" style={{ background: c.value, border: '1px solid #5a5f75' }} />
                                        <span style={{ fontSize: 8, color: currS.colore_interno === c.value ? '#eae8e3' : '#8b90a8' }}>{c.label}</span>
                                      </button>
                                    ))}
                                  </div>
                                </div>
                                <SelectField label="Finitura" value={currS.finitura} onChange={v => upd('finitura', v)} options={[
                                  { value: 'liscio', label: 'Liscio' }, { value: 'venato_legno', label: 'Effetto Legno' },
                                  { value: 'spazzolato', label: 'Spazzolato' }, { value: 'opaco', label: 'Opaco' }, { value: 'lucido', label: 'Lucido' },
                                ]} />
                              </div>
                            )}
                            ...
                            {configTab === 'vetro' && (
                              <div className="space-y-3">
                                <div>
                                  <label style={{ fontSize: 9, color: '#5a5f75', fontWeight: 600, textTransform: 'uppercase' as const, letterSpacing: 0.5, marginBottom: 6, display: 'block' }}>TIPO VETRO</label>
                                  <div className="grid grid-cols-2 gap-1.5">
                                    {[{ value: 'doppio', label: 'Doppio Vetro', desc: '2 lastre' }, { value: 'triplo', label: 'Triplo Vetro', desc: '3 lastre' }].map(v => (
                                      <button key={v.value} onClick={() => {
                                        upd('tipo_vetro', v.value)
                                        setTimeout(() => upd('composizione_vetro', v.value === 'triplo' ? '4/16/4/16/4' : '4/16/4'), 100)
                                      }} className="p-3 rounded-lg text-center" style={{
                                        background: currS.tipo_vetro === v.value ? '#5b9cf615' : '#0d0f17',
                                        border: `1px solid ${currS.tipo_vetro === v.value ? '#5b9cf640' : '#1a1e2e'}`,
                                        cursor: 'pointer',
                                      }}>
                                        <div style={{ fontSize: 18 }}>💎</div>
                                        <div style={{ fontSize: 10, color: currS.tipo_vetro === v.value ? '#5b9cf6' : '#8b90a8', fontWeight: 700, marginTop: 4 }}>{v.label}</div>
                                        <div style={{ fontSize: 8, color: '#5a5f75' }}>{v.desc}</div>
                                      </button>
                                    ))}
                                  </div>
                                </div>
                                <InputField label="Composizione (mm)" value={currS.composizione_vetro} onChange={v => upd('composizione_vetro', v)} placeholder="es. 4/16/4" />
                                <SelectField label="Gas Camera" value={currS.gas_camera} onChange={v => upd('gas_camera', v)} options={[
                                  { value: 'aria', label: 'Aria' }, { value: 'argon', label: 'Argon (standard)' }, { value: 'kripton', label: 'Kripton (premium)' },
                                ]} />
                                <div className="space-y-2">
                                  {[
                                    { field: 'vetro_basso_emissivo', label: 'Basso Emissivo (Low-E)' },
                                    { field: 'vetro_temperato', label: 'Temperato (sicurezza)' },
                                    { field: 'vetro_stratificato', label: 'Stratificato (antieffrazione)' },
                                  ].map(opt => (
                                    <div key={opt.field} className="flex items-center gap-3 p-2 rounded-lg" style={{ background: '#0d0f17' }}>
                                      <button onClick={() => upd(opt.field, !(currS as any)[opt.field])}
                                        style={{ width: 32, height: 18, borderRadius: 9, border: 'none', cursor: 'pointer', position: 'relative', background: (currS as any)[opt.field] ? '#5b9cf6' : '#252a3d' }}>
                                        <div style={{ width: 14, height: 14, borderRadius: 7, background: '#eae8e3', position: 'absolute', top: 2, left: (currS as any)[opt.field] ? 16 : 2, transition: 'left 0.2s' }} />
                                      </button>
                                      <span style={{ fontSize: 10, color: '#8b90a8' }}>{opt.label}</span>
                                    </div>
                                  ))}
                                </div>
                                <div>
                                  <label style={{ fontSize: 9, color: '#5a5f75', fontWeight: 600, display: 'block', marginBottom: 4 }}>Ug VETRO (W/m²K)</label>
                                  <input type="number" value={currS.ug_valore} onChange={e => upd('ug_valore', parseFloat(e.target.value) || 0)} step={0.1}
                                    style={{ width: '100%', background: '#0d0f17', border: '1px solid #252a3d', borderRadius: 8, padding: '6px 8px', fontSize: 12, color: '#eae8e3', fontFamily: 'monospace', outline: 'none' }} />
                                </div>
                              </div>
                            )}

                            {/* ---- FERRAMENTA ---- */}
                            {configTab === 'ferramenta' && (
                              <div className="space-y-3">
                                <SelectField label="Tipo Ferramenta" value={currS.tipo_ferramenta} onChange={v => upd('tipo_ferramenta', v)} options={[
                                  { value: 'standard', label: 'Standard' }, { value: 'comfort', label: 'Comfort (microventilazione)' }, { value: 'sicurezza', label: 'Sicurezza (antieffrazione)' },
                                ]} />
                                {currS.tipo_ferramenta === 'sicurezza' && (
                                  <SelectField label="Classe Antieffrazione" value={currS.classe_antieffrazione} onChange={v => upd('classe_antieffrazione', v)} options={[
                                    { value: 'RC1', label: 'RC1 - Base' }, { value: 'RC2', label: 'RC2 - Standard' }, { value: 'RC3', label: 'RC3 - Elevata' },
                                  ]} />
                                )}
                                <SelectField label="Maniglia" value={currS.maniglia} onChange={v => upd('maniglia', v)} options={[
                                  { value: 'standard', label: 'Standard' }, { value: 'design', label: 'Design' }, { value: 'con_chiave', label: 'Con chiave' }, { value: 'minimale', label: 'Minimale' },
                                ]} />
                                <div>
                                  <label style={{ fontSize: 9, color: '#5a5f75', fontWeight: 600, textTransform: 'uppercase' as const, marginBottom: 6, display: 'block' }}>COLORE MANIGLIA</label>
                                  <div className="flex gap-2 flex-wrap">
                                    {[
                                      { value: '#C0C0C0', label: 'Argento' }, { value: '#FFD700', label: 'Oro' },
                                      { value: '#3B3B3B', label: 'Antracite' }, { value: '#FFFFFF', label: 'Bianco' },
                                      { value: '#1C1C1C', label: 'Nero' }, { value: '#8B6914', label: 'Bronzo' },
                                    ].map(c => (
                                      <button key={c.value} onClick={() => upd('colore_maniglia', c.value)}
                                        className="flex items-center gap-1 p-1.5 rounded" style={{
                                          background: currS.colore_maniglia === c.value ? '#5b9cf615' : '#0d0f17',
                                          border: `1px solid ${currS.colore_maniglia === c.value ? '#5b9cf640' : '#1a1e2e'}`,
                                          cursor: 'pointer',
                                        }}>
                                        <div className="w-3 h-3 rounded-full" style={{ background: c.value, border: '1px solid #5a5f75' }} />
                                        <span style={{ fontSize: 8, color: '#8b90a8' }}>{c.label}</span>
                                      </button>
                                    ))}
                                  </div>
                                </div>
                                <SelectField label="Cerniere" value={currS.cerniere} onChange={v => upd('cerniere', v)} options={[
                                  { value: 'standard', label: 'Standard (visibili)' }, { value: 'a_scomparsa', label: 'A scomparsa' },
                                ]} />
                              </div>
                            )}
                            ...
                            {configTab === 'accessori' && (
                              <div className="space-y-2">
                                {[
                                  { field: 'cassonetto', label: '🔲 Cassonetto', sub: 'tipo_cassonetto', options: ['tradizionale', 'monoblocco', 'renotherm'] },
                                  { field: 'avvolgibile', label: '🪟 Avvolgibile', sub: 'tipo_avvolgibile', options: ['pvc', 'alluminio', 'coibentato'] },
                                  { field: 'zanzariera', label: '🦟 Zanzariera', sub: 'tipo_zanzariera', options: ['laterale', 'verticale', 'plisse'] },
                                  { field: 'persiana', label: '🏠 Persiana', sub: 'tipo_persiana', options: ['alluminio', 'legno', 'pvc'] },
                                ].map(acc => (
                                  <div key={acc.field} className="p-2 rounded-lg" style={{ background: '#0d0f17' }}>
                                    <div className="flex items-center gap-3">
                                      <button onClick={() => upd(acc.field, !(currS as any)[acc.field])}
                                        style={{ width: 32, height: 18, borderRadius: 9, border: 'none', cursor: 'pointer', position: 'relative', background: (currS as any)[acc.field] ? '#5b9cf6' : '#252a3d' }}>
                                        <div style={{ width: 14, height: 14, borderRadius: 7, background: '#eae8e3', position: 'absolute', top: 2, left: (currS as any)[acc.field] ? 16 : 2, transition: 'left 0.2s' }} />
                                      </button>
                                      <span style={{ fontSize: 10, color: '#8b90a8' }}>{acc.label}</span>
                                    </div>
                                    {(currS as any)[acc.field] && (
                                      <div className="flex gap-1 mt-2 ml-10">
                                        {acc.options.map(o => (
                                          <button key={o} onClick={() => upd(acc.sub, o)} style={{
                                            padding: '2px 8px', borderRadius: 4, fontSize: 9, border: 'none', cursor: 'pointer',
                                            background: (currS as any)[acc.sub] === o ? '#5b9cf630' : '#181b27',
                                            color: (currS as any)[acc.sub] === o ? '#5b9cf6' : '#5a5f75',
                                          }}>{o}</button>
                                        ))}
                                      </div>
                                    )}
                                ...
                                {/* Motorizzato */}
                                <div className="flex items-center gap-3 p-2 rounded-lg" style={{ background: '#0d0f17' }}>
                                  <button onClick={() => upd('motorizzato', !currS.motorizzato)}
                                    style={{ width: 32, height: 18, borderRadius: 9, border: 'none', cursor: 'pointer', position: 'relative', background: currS.motorizzato ? '#5b9cf6' : '#252a3d' }}>
                                    <div style={{ width: 14, height: 14, borderRadius: 7, background: '#eae8e3', position: 'absolute', top: 2, left: currS.motorizzato ? 16 : 2, transition: 'left 0.2s' }} />
                                  </button>
                                  <span style={{ fontSize: 10, color: '#8b90a8' }}>⚡ Motorizzato</span>
                                </div>
                                {/* Davanzali */}
                                <div className="flex gap-3">
                                  {['davanzale_interno', 'davanzale_esterno'].map(d => (
                                    <div key={d} className="flex items-center gap-2 p-2 rounded-lg flex-1" style={{ background: '#0d0f17' }}>
                                      <button onClick={() => upd(d, !(currS as any)[d])}
                                        style={{ width: 32, height: 18, borderRadius: 9, border: 'none', cursor: 'pointer', position: 'relative', background: (currS as any)[d] ? '#5b9cf6' : '#252a3d' }}>
                                        <div style={{ width: 14, height: 14, borderRadius: 7, background: '#eae8e3', position: 'absolute', top: 2, left: (currS as any)[d] ? 16 : 2, transition: 'left 0.2s' }} />
                                      </button>
                                      <span style={{ fontSize: 9, color: '#8b90a8' }}>{d === 'davanzale_interno' ? 'Dav. Int.' : 'Dav. Est.'}</span>
                                    </div>
                                  ))}
                                </div>
                                <SelectField label="Soglia" value={currS.soglia} onChange={v => upd('soglia', v)} options={[
                                  { value: '', label: 'Nessuna' }, { value: 'standard', label: 'Standard' },
                                  { value: 'ribassata', label: 'Ribassata' }, { value: 'complanare', label: 'Complanare' },
                                ]} />
                                <InputField label="Note" value={currS.note || ''} onChange={v => upd('note', v)} placeholder="Note aggiuntive..." />
                              </div>
                            )}
                          </div>
                        )
                      })() : (
                        <div className="flex flex-col items-center justify-center py-12" style={{ color: '#5a5f75' }}>
                          <p style={{ fontSize: 11 }}>Seleziona un serramento a sinistra</p>
                        </div>
                      )}
                    </Card>
                  </div>
