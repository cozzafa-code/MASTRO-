...
export default function Dashboard() {
  ...
  async function loadCalendario() {
    ...
    const { data } = await supabase.from('eventi').select('*, cliente:clienti(*)').eq('azienda_id', AZIENDA_ID).gte('data', d1.toISOString().split('T')[0]).lt('data', d2.toISOString().split('T')[0]).order('ora_inizio', { ascending: true })
  ...
  return (
    <>
      <MobileLayout />
      <div className="hidden lg:flex min-h-screen" style={{ background: BG.page, color: TX.text }}>
        {/* Sidebar */}
        <aside className="w-56 flex flex-col" style={{ background: BG.card, borderRight: `1px solid ${TX.border}`, boxShadow: isDark ? '2px 0 8px rgba(0,0,0,0.2)' : '2px 0 8px rgba(0,0,0,0.03)' }}>
          <div className="px-5 py-5">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl flex items-center justify-center text-lg font-extrabold" style={{ background: `linear-gradient(135deg, ${AC}, ${TH.purple})`, color: '#fff' }}>M</div>
              <div>
                <h1 className="text-base font-bold tracking-tight" style={{ color: TX.text }}>MASTRO</h1>
                <p style={{ fontSize: 8, color: TX.textMuted, letterSpacing: 2, textTransform: 'uppercase' as const }}>Walter Cozza Srl</p>
              </div>
            </div>
          </div>
          <nav className="flex-1 px-3 space-y-1">
            {tabs.map(tab => (
              <button key={tab.id} onClick={() => { setActiveTab(tab.id); if (tab.id === 'contabilita') loadContabilita(); if (tab.id === 'magazzino') loadMagazzino(); if (tab.id === 'produzione') loadProduzione(); if (tab.id === 'clienti') loadClienti(); if (tab.id === 'calendario') loadCalendario() }}
                ...
                style={{ background: activeTab === tab.id ? AC + '10' : 'transparent', color: activeTab === tab.id ? AC : TX.textSec }}>
                <Ic n={tab.icon} s={16} c={activeTab === tab.id ? AC : TX.textMuted}/>
                {tab.label}
                {activeTab === tab.id && <div className="ml-auto w-1.5 h-1.5 rounded-full" style={{ background: AC }} />}
              </button>
            ))}
          </nav>
          {/* User profile */}
          <div className="p-3 mt-auto" style={{ borderTop: `1px solid ${TX.border}` }}>
            <div className="flex items-center gap-2.5 p-2 rounded-lg" style={{ background: BG.input }}>
              <div style={{ width: 32, height: 32, borderRadius: '50%', background: `linear-gradient(135deg, ${userConfig.color}, ${userConfig.color}80)`,
                display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 800, color: '#fff', flexShrink: 0 }}>
                {profilo?.avatar || (profilo?.nome?.[0] || '') + (profilo?.cognome?.[0] || '')}
              </div>
              <div className="flex-1 min-w-0">
                <div style={{ fontSize: 11, fontWeight: 700, color: TX.text, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{profilo?.nome} {profilo?.cognome}</div>
                <div className="flex items-center gap-1.5">
                  <span style={{ fontSize: 10 }}>{userConfig.icon}</span>
                  <span style={{ fontSize: 8, fontWeight: 700, color: userConfig.color, textTransform: 'uppercase', letterSpacing: 0.5 }}>{userLicenza}</span>
                </div>
              </div>
              <button onClick={handleLogout} title="Esci"
                style={{ padding: 4, background: 'transparent', border: 'none', cursor: 'pointer', opacity: 0.5 }}>
                <Ic n="x" s={14} c={TX.textMuted}/>
              </button>
            </div>
            <div className="text-center mt-2">
              <p style={{ fontSize: 8, color: TX.textMuted }}>MASTRO ERP v1.0</p>
            </div>
          </div>
        </aside>
        ...
        <main className="flex-1 overflow-auto">
          <header className="px-8 py-5 flex items-center justify-between" style={{ borderBottom: `1px solid ${TX.border}`, background: BG.card }}>
            <div>
              <h2 className="text-xl font-bold flex items-center gap-2" style={{ color: TX.text }}>
                <Ic n={tabs.find(t => t.id === activeTab)?.icon || 'home'} s={20} c={TH.amber}/>
                {tabs.find(t => t.id === activeTab)?.label}
              </h2>
              <p style={{ fontSize: 11, color: TX.textMuted }}>{oggi}</p>
            </div>
            <div className="flex items-center gap-3">
              <button onClick={() => setShowGlobalSettings(true)} className="px-3 py-2 rounded-lg text-xs flex items-center gap-1.5"
                style={{ background: AC + '10', border: `1px solid ${AC}30`, color: AC, cursor: 'pointer', fontWeight: 600 }}>
                <Ic n="settings" s={13} c={AC}/> Tema
              </button>
              <button className="px-3 py-2 rounded-lg text-xs flex items-center gap-1.5" style={{ background: BG.input, border: `1px solid ${TX.border}`, color: TX.textSec }}>
                <Ic n="bell" s={13} c={TX.textSec}/> Notifiche
              </button>
              <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold" style={{ background: AC + '15', color: AC }}>WC</div>
            </div>
          </header>
          <div className="p-8">
        ...
        {showGlobalSettings && (
          <div style={{ position: 'fixed', inset: 0, zIndex: 9998, display: 'flex', justifyContent: 'flex-end' }}
            ...
            <div style={{ width: 400, background: BG.card, height: '100%', overflowY: 'auto', borderLeft: `1px solid ${TX.border}`, boxShadow: '-8px 0 30px rgba(0,0,0,0.15)' }}
              onClick={(e) => e.stopPropagation()}>
              <div className="p-5 flex items-center justify-between" style={{ borderBottom: `1px solid ${TX.border}`, position: 'sticky', top: 0, background: BG.card, zIndex: 10 }}>
                <div>
                  <h3 style={{ fontSize: 16, fontWeight: 800, color: TX.text }}>Personalizza MASTRO</h3>
                  <p style={{ fontSize: 10, color: TX.textMuted }}>Il tema si applica a tutte le sezioni</p>
                </div>
                <button onClick={() => setShowGlobalSettings(false)} style={{ padding: 6, background: BG.input, border: `1px solid ${TX.border}`, borderRadius: 8, cursor: 'pointer' }}>
                  <Ic n="x" s={14} c={TX.textMuted}/>
                </button>
              </div>
              <div className="p-5 space-y-6">
                ...
                {/* ── PRESET SALVATI ── */}
                <div>
                  <h4 style={{ fontSize: 10, fontWeight: 700, color: TX.textMuted, textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 10 }}>
                    💾 Preset Salvati
                  </h4>
                  <div className="space-y-1.5 mb-3">
                    {savedPresets.map((p, i) => (
                      <div key={i} className="flex items-center gap-2 p-2 rounded-lg cursor-pointer"
                        style={{ background: BG.input, border: `1px solid ${TX.border}` }}
                        onClick={() => loadPreset(p)}>
                        <div style={{ width: 14, height: 14, borderRadius: 4, background: p.style.accent, flexShrink: 0 }} />
                        <div style={{ width: 14, height: 14, borderRadius: 4, background: bgThemes[p.style.bgTheme]?.page || '#F7F8FA', border: '1px solid #ddd', flexShrink: 0 }} />
                        <span style={{ fontSize: 12, fontWeight: 600, color: TX.text, flex: 1 }}>{p.name}</span>
                        <span style={{ fontSize: 8, color: TX.textMuted }}>{bgThemes[p.style.bgTheme]?.label}</span>
                        <button onClick={(e) => { e.stopPropagation(); deletePreset(p.name) }}
                          style={{ padding: 2, background: 'transparent', border: 'none', cursor: 'pointer', opacity: 0.4 }}>
                          <Ic n="x" s={10} c={TX.textMuted}/>
                        </button>
                      </div>
                    ))}
                  </div>
                  <div className="flex gap-2">
                    <input value={presetName} onChange={(e) => setPresetName(e.target.value)} placeholder="Nome preset..."
                      onKeyDown={(e) => { if (e.key === 'Enter') savePreset(presetName) }}
                      style={{ flex: 1, padding: '6px 10px', background: BG.input, border: `1px solid ${TX.border}`, borderRadius: 6, fontSize: 11, color: TX.text, outline: 'none' }} />
                    <button onClick={() => savePreset(presetName)}
                      style={{ padding: '6px 12px', background: AC, border: 'none', borderRadius: 6, fontSize: 10, fontWeight: 700, color: '#fff', cursor: 'pointer' }}>
                      Salva
                    </button>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {[
                      { id: '#D97706', label: 'MASTRO', desc: 'Originale' },
                      { id: '#3B82F6', label: 'Ocean', desc: 'Professionale' },
                      { id: '#059669', label: 'Forest', desc: 'Naturale' },
                      { id: '#7C3AED', label: 'Royal', desc: 'Elegante' },
                      { id: '#DC2626', label: 'Sunset', desc: 'Energico' },
                      { id: '#EC4899', label: 'Rose', desc: 'Moderno' },
                      { id: '#0D9488', label: 'Teal', desc: 'Fresco' },
                      { id: '#4F46E5', label: 'Indigo', desc: 'Sofisticato' },
                    ].map(t => (
                      <button key={t.id} onClick={() => setDashStyle(prev => ({ ...prev, accent: t.id }))}
                        style={{ padding: '8px 4px', border: AC === t.id ? `2px solid ${t.id}` : `1px solid ${TX.border}`, borderRadius: 8,
                          background: AC === t.id ? t.id + '08' : BG.input, cursor: 'pointer', textAlign: 'center' }}>
                        <div style={{ width: 20, height: 20, borderRadius: '50%', background: t.id, margin: '0 auto 4px' }} />
                        <div style={{ fontSize: 9, fontWeight: 600, color: AC === t.id ? t.id : TX.text }}>{t.label}</div>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {Object.entries(bgThemes).map(([id, t]) => {
                      const dk = ['charcoal', 'midnight', 'forest'].includes(id)
                      return (
                        <button key={id} onClick={() => setDashStyle(prev => ({ ...prev, bgTheme: id }))}
                          style={{ padding: '8px 4px', border: dashStyle.bgTheme === id ? `2px solid ${AC}` : `1px solid ${TX.border}`,
                            borderRadius: 8, background: dashStyle.bgTheme === id ? AC + '08' : BG.input, cursor: 'pointer', textAlign: 'center' }}>
                          <div style={{ width: 32, height: 20, borderRadius: 4, margin: '0 auto 4px', background: t.page, border: `1px solid ${dk ? '#444' : '#ddd'}`, position: 'relative', overflow: 'hidden' }}>
                            <div style={{ position: 'absolute', bottom: 0, left: 2, right: 2, height: 10, borderRadius: '3px 3px 0 0', background: t.card }} />
                          </div>
                          <div style={{ fontSize: 8, fontWeight: 600, color: dashStyle.bgTheme === id ? AC : TX.text }}>{t.label}</div>
                        </button>
                      )
                    })}
                  </div>
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    {[
                      { id: 'sharp', label: 'Squadrato', r: 4 },
                      { id: 'rounded', label: 'Arrotondato', r: 12 },
                      { id: 'pill', label: 'Morbido', r: 20 },
                    ].map(s => (
                      <button key={s.id} onClick={() => setDashStyle(prev => ({ ...prev, radius: s.id }))}
                        className="flex-1 p-3" style={{ border: dashStyle.radius === s.id ? `2px solid ${AC}` : `1px solid ${TX.border}`,
                          borderRadius: 8, background: dashStyle.radius === s.id ? AC + '08' : BG.input, cursor: 'pointer', textAlign: 'center' }}>
                        <div style={{ width: 28, height: 20, borderRadius: s.r, border: `2px solid ${dashStyle.radius === s.id ? AC : TX.textMuted}`, margin: '0 auto 4px' }} />
                        <div style={{ fontSize: 9, fontWeight: 600, color: dashStyle.radius === s.id ? AC : TX.text }}>{s.label}</div>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    {[
                      ...
                      <button key={d.id} onClick={() => setDashStyle(prev => ({ ...prev, density: d.id }))}
                        className="flex-1 p-3" style={{ border: dashStyle.density === d.id ? `2px solid ${AC}` : `1px solid ${TX.border}`,
                          borderRadius: 8, background: dashStyle.density === d.id ? AC + '08' : BG.input, cursor: 'pointer', textAlign: 'center' }}>
                        <div style={{ fontSize: 10, fontWeight: 600, color: dashStyle.density === d.id ? AC : TX.text }}>{d.label}</div>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                {/* ── ANTEPRIMA ── */}
                <div className="p-3 rounded-xl" style={{ background: BG.page, border: `1px solid ${TX.border}` }}>
                  <p style={{ fontSize: 8, color: TX.textMuted, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 6, fontWeight: 700 }}>Anteprima</p>
                  <div style={{ background: BG.card, padding: 12, borderRadius: rd, border: `1px solid ${TX.border}`, boxShadow: isDark ? '0 1px 4px rgba(0,0,0,0.3)' : TH.shadow }}>
                    <div className="flex items-center justify-between mb-2">
                      <span style={{ fontSize: 10, fontWeight: 700, color: AC }}>WC-0001</span>
                      <span style={{ fontSize: 9, padding: '2px 6px', borderRadius: 4, background: TH.green + '15', color: TH.green }}>Attiva</span>
                    </div>
                    <div style={{ fontSize: 12, fontWeight: 600, color: TX.text, marginBottom: 2 }}>Commessa di Esempio</div>
                    <div style={{ fontSize: 10, color: TX.textSec }}>Mario Rossi · 12.000,00 €</div>
                    <div className="flex gap-1 mt-2">
                      {['#4CAF50','#4CAF50','#4CAF50',AC,'transparent','transparent','transparent'].map((c,i) => (
                        <div key={i} style={{ flex: 1, height: 4, borderRadius: 2, background: c === 'transparent' ? TX.bgHover : c, opacity: c === 'transparent' ? 0.3 : 0.8 }} />
                      ))}
                    </div>
                  </div>
                </div>
